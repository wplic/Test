...
<%
		...
		TclientTzcl tclientTzcl = null;
		if (ydbgId != null && !"".equals(ydbgId))
		{
			tclientTzcl = tclientYdbgService.getTclientTzclById(Long.parseLong(ydbgId));
		}
		...
		if (tclientTzcl != null)
		{
			if ( tclientTzcl.getConnectId( ) != null && !"".equals(tclientTzcl.getConnectId( )))
			{
				txtblb = lobService.getTextBlobById(Long.parseLong(tclientTzcl.getConnectId()));
				strContext = new String(txtblb.getSource(),"UTF-8");
			}
			if (tclientTzcl.getFjId() != null && !"".equals(tclientTzcl.getFjId()))
			{
				fileBlob = lobService.getFileBlobById(Long.parseLong(tclientTzcl.getFjId()));
			}
			if (!"".equals(tclientTzcl.getRecuserId()))
			{
				userBasicInfo = userService.getUserBasicInfoByUserId(tclientTzcl.getRecuserId());
			}
...
<div class="main">
  ...
  <div class="bg_f5 p8">
	<div class="tlbg3">
        	...
        	<%
        			if (tclientTzcl != null && tclientTzcl.getTitle() != null)
        			{
        					%>
        							<%= StringUtil.subString(tclientTzcl.getTitle(),80,"...")  %>
        					<%
        			}
        	 %>
        	</p>
  <p class="textTitle2" align="center"> 日期：<%= ( (tclientTzcl != null &&tclientTzcl.getRecDate() == null) ?"":sdf.format(tclientTzcl.getRecDate())) %> 作者：客服中心
  						
  					</p>
    <%if(tclientTzcl != null && tclientTzcl.getDigest() != null) {%>
   <div class="newstextArea">
   <p class="newstextAreatop"></p>
   <strong >【摘要】</strong><br/>
   <div>
  <%=tclientTzcl.getDigest()%>
  </div>
