...
<%@page import="com.cssweb.gazxfund.pojo.NewFundInfo"%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<form action="${contextPath}/newFundInfoAction.do?method=getOpenNewFundInfoList" id="fundValueForm" name="fundValueForm" method="post">
    <div class="blank5"></div>
    <ul class="gsyw_box2">
        	<li class="active red bold" id="tagtabs11" onmouseover="mySwitchNews('tagtabs11')"><a href="${contextPath}/newFundInfoAction.do?method=getOpenNewFundInfoList&flagJjlx=newopen" target="fundSort">开放式基金</a></li>
            <li class=" white bold" id="tagtabs12" onmouseover="mySwitchNews('tagtabs12')"><a href="${contextPath}/newFundInfoAction.do?method=getOpenNewFundInfoList&flagJjlx=newclose" target="fundSort">封闭式基金</a></li>
    </ul>
 <div class="blank5"></div>
    <div class="query_bg1">
	...
	<table border="1" class="lh24 center" bordercolor="#cccccc" width="100%" id="tabs11">
      <tr>
        ...
        <td class="tab_th1 bold" colspan="2">发行期 </td>
      					...
      					for (int i = 0 ; i < list.size() ; i ++)
      					{
      							...
      							if ( (i % 2) != 0)
      							{
	      							%>
	      								<tr>
									        <td><%= tmp %></td>
									        <td><%= newFundInfo.getJjdm() %></td>
									        <td><%= newFundInfo.getJjjc() %></td>
									        <td><%=newFundInfo.getFxQSrq() ==null?"": sdf.format(newFundInfo.getFxQSrq()) %> </td>
									        <td><%=newFundInfo.getFxJZrq() ==null?"": sdf.format(newFundInfo.getFxJZrq()) %> </td>
									        <td><%= newFundInfo.getGsmc()==null?"":newFundInfo.getGsmc() %></td>
									    </tr>	
	      							<%
      							}
      							else
