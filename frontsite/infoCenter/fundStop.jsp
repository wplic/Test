...
<%@page import="com.cssweb.gazxfund.pojo.FundStopDeal"%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<form action="${contextPath }/fundStopDealAction.do?method=getFundStopDealList" id="fundValueForm" name="fundValueForm" method="post">
    <div class="blank5"></div>
    <ul class="gsyw_box2">
        	<li class="active red bold" id="tagtabs11" onmouseover="mySwitchNews('tagtabs11')"><a href="${contextPath }/fundStopDealAction.do?method=getFundStopDealList&flagJjlx=newopen" target="fundSort">开放式基金</a></li>
            <li class=" white bold" id="tagtabs12" onmouseover="mySwitchNews('tagtabs12')"><a href="${contextPath }/fundStopDealAction.do?method=getFundStopDealList&flagJjlx=newclose" target="fundSort">封闭式基金</a></li>
    </ul>
 <div class="blank5"></div>
    <div class="query_bg1">
	...
	<table border="1" class="lh24 center" bordercolor="#cccccc" width="100%" id="tabs11">
      <tr>
        ...
        <td class="tab_th1 bold">基金类型 </td>
        <td class="tab_th1 bold" >当前净值</td>
       	<td class="tab_th1 bold">暂停原因</td>
       	<td class="tab_th1 bold" > 简称拼音</td>
      					...
      					for (int i = 0 ; i < list.size() ; i ++)
      					{
      							...
      							if ( (i % 2) != 0)
      							{
	      							%>
	      								<tr>
									        <td><%= tmp %></td>
									        <td><%= fundStopDeal.getJjdm() %></td>
									        <td><%= fundStopDeal.getJjjc() %></td>
									        <td><%=fundStopDeal.getJjlx() ==null?"": fundStopDeal.getJjlx() %> </td>
									        <td><%=fundStopDeal.getDwjz()==null?"":fundStopDeal.getDwjz()  %> </td>
									        <td><%= fundStopDeal.getZtyy()==null?"":fundStopDeal.getZtyy() %></td>
									        <td><%= fundStopDeal.getGpjcpy()==null?"":fundStopDeal.getGpjcpy() %></td>
									    </tr>	
	      							<%
      							}
      							else
