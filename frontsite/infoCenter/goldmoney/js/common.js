...
function redirectUrl(formName){
	if(formName == "xxss"){
		...
		var xxssK = $(this).find(".xxssK").val();
		var xxssSel = $(this).find(".xxssSel").val();
		//window.open("/fzlm/search/?k="+ xxssK +"&sslm="+ xxssSel);
		var url = "/fzlm/search/?k="+ escape(xxssK);
		var cid = "0";
		if (xxssSel!="00")
		{
			cid = xxssSel;
	...
	else if(formName == "ccss"){
		//document.location.href = url;
		var ccssK = $(this).find(".ccssK").val();
		var ccssSel = $(this).find(".ccssSel").val();//地区
		var ccssSel02 = $(this).find(".ccssSel02").val();//状态
	
		var url = "/fzlm/search/?k="+ escape(ccssK);
	
		var where = "";
		if (ccssSel!="00")
		{
			 where = "fdExtPro=" + ccssSel;
			 url += "&where=" + escape(where);
		}
