...
<form name="searchForm" id="searchForm"	action="${contextPath}/inverstAdvise/web/InverstAdviserAction.go?function=ShowFronArticleItems&owner_id=${requestScope.dataitem.id }" method="post">
...
<!--请从此处复制代码开始-->
  ...
  <img class="img" alt="${requestScope.dataitem.name }"  src="${contextPath}/inverstAdvise/load_image.jsp?mpic=${requestScope.dataitem.id}"/>
  <div class="tgzl_con3_n">${requestScope.dataitem.name }</div>
<div align="center" style="color:#003366; background:url('${contextPath}/inverstAdvise/images/tgzl_06.gif')  no-repeat;">
  ${fn:substring( requestScope.dataitem.positionname,0,11)}
  </div>
  <div class="tgzl_con3_c">${requestScope.dataitem.introduction}</div>
  </div>
