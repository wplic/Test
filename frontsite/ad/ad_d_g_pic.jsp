...
//取得显示图片数量
 ...
 if(adList!=null&&adList.size()>0)
   countsp=String.valueOf(adList.size());
if(!countsp.equals("0"))
{
//取得广告位编码
...
<% 
	...
	<%
		//对取得的广告位进行处理
		for(int i=0;i<Integer.parseInt(countsp);i++){
			adManger=(AdManger)adList.get(i);
			long picId = adManger.getPicId();
			String url = adManger.getUrl();
	%>
			imgp[<%=i+1%>]=new Image();
			imgp[<%=i+1%>].src='/share/showUploadImage.jsp?id=<%=picId%>';
			urlp[<%=i+1%>]=new Image();
			urlp[<%=i+1%>].src='<%=url%>';
	<%
		}
...
%>
 <SCRIPT src="/js/focus3.js"></SCRIPT> </td>
        </tr>
      </table>
      <!-- 图片广告切换 End -->
    </td>
