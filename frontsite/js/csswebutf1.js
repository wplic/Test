...
//window.ie=Fun.ie;
//window.magicA=Fun.toarr;
window.magic=function(id,tag)
{
...
window.Effect=
{
	...
	SwitchCss:function(id,tag,cName,sp){var c=magic(id,tag);sp=sp||2;for(var i=0,n=c.length;i<n;i+=sp){c[i].className=cName;}},
	...
	{
		var t=tit.split('/'),b=box.split("/"),ts=magic(t[0],t[1]),bs=magic(b[0],b[1]),s=s||"onmouseover",now=show=show||0,c;
	...
	{
		var c=magic(id,tag),now=(show!=-1)?show:0,o;for(var i=0;i<c.length;i++){c[i].old=c[i].className.replace(cName,"");c[i].seal=false;reg(i);}
	...
	{
		var au=magic(foc,"a"),bimgs=magic(foc,"img[alt!=simg]"),ba=magic(fbimg,"dt")[0].find("a")[0],now=1,tm,len=au.length;
		...
		var bp=magic(fbimg,"dd")[0].find("ul")[0];for(var i=0;i<len;i++){bp.innerHTML+="<li>"+(i+1)+"</li>";}
		var bps=bp.find("li");bps[0].className="show";function pfn(i){bps[i].onclick=function(){go(i);}};for(var i=0;i<len;i++){pfn(i);}
		if(fsimg){var simgs=magic(foc,"img[alt=simg]"),simg=magic(fsimg);for(var i=0;i<len;i++){simg.innerHTML+="<dl><dt><img src="+simgs[i].src+" /></dt><dd>"+au[i].title+"</dd></dl>";}
		var sx=magic(simg,"dl");sx[0].className="show";function sfn(i){sx[i].onclick=function(){go(i);}};for(var i=0;i<len;i++){sfn(i);}}
		if(ftext){var fte=magic(ftext);fte.innerHTML="<dt><a href="+au[0].href+">"+bimgs[0].alt+"</a></dt><dd>"+au[0].title+"</dd>";}
		...
		if(ftext){magic(ftext).innerHTML="<dt><a href="+au[now].href+">"+bimgs[now].alt+"</a></dt><dd>"+au[now].title+"</dd>";}(now<len-1)?now++:now=0;}
	...
	{
		var c,mous=false,fg=tag.split('/'),o=magic(id),as=o.find(fg[1]),fx=(path=="scrollRight"||path=="scrollLeft")?"scrollLeft":"scrollTop",ow=fx=="scrollTop"?as[0].h():as[0].w();
		...
		if(pgtime==null)
		{
			...
			if(upbt){magic(upbt).onmousedown=function(){down();rate+=3;};magic(upbt).onmouseup=function(){rate=oldra;};}
			if(downbt){magic(downbt).onmousedown=function(){up();rate+=3;};magic(downbt).onmouseup=function(){rate=oldra;};}
		...
		else
		{
			...
			if(upbt){magic(upbt).onmousedown=function(){clearInterval(d);go_to(true);d=setInterval(function(){go_to(true);},pgtime);}}
			if(downbt){magic(downbt).onmousedown=function(){clearInterval(d);go_to(false);d=setInterval(function(){go_to(false);},pgtime);}}
	...
	Drift:function(id,tag,ctid,toppx,leftpx){
		...
		function mDir(i){
			...
			Dom.addEvent(obj[i],"mousemove",function(e){
				...
				magic(ctid).style.top =y+toppx+'px'; 
				magic(ctid).style.left =x+leftpx+'px';
			});
		}
	},
	...
	{
		var o=magic(id),over=0,ar=Array(),len=mx.length,temp;
	...
	FixedMove:function(id,x,y,content)
	{
		...
		ids=magic(id);setInterval(plays,10);	
	...
	SelectList:function(id,putid,now){
		var inbor=magic(id),indt=magic(id,"dt")[0],indd=magic(id,"dd")[0],listi=magic(id,"li");if(putid){var putid=magic(putid);}
		...
		function isel(i){
			if(now && now==listi[i].getAttribute("value")){
				if(putid){indt.innerHTML=listi[i].innerHTML;putid.value=listi[i].getAttribute("value");}else{indt.innerHTML=magic(listi[i],"a")[0].innerHTML;}
			...
			listi[i]["onclick"]=function(){
				if(putid){indt.innerHTML=this.innerHTML;putid.value=this.getAttribute("value");}else{indt.innerHTML=magic(listi[i],"a")[0].innerHTML;}
				for(var num=0;num<listi.length;num++){listi[num].id=(i==num)?"open":"";}
			}
		}
	},
	...
	openwin:function(id,path,dist){
		...
		var objid=magic(id);
		magic("winmask").style.height=document.documentElement.scrollHeight+"px"; //设遮罩层高度
		objid.style.display=magic("winmask").style.display=(objid.style.display=="none")?"block":"none"; //判断窗口显示隐藏  判断遮罩层显示隐藏
	...
	ScrollBar:function(conid,barid){
		var dragid=magic(barid,"p")[0]; //滚动条
		var conObg=magic(conid);
		var tops;
		dragid.style.height=(magic(barid).offsetHeight*conObg.offsetHeight/conObg.scrollHeight)+"px"; //算出滚动块的长度
		conObg.onscroll=function(){dragid.style.top=(conObg.scrollTop / conObg.scrollHeight * magic(barid).offsetHeight)+"px";};
		dragid.onmousedown=function(e){
			...
			winobj.onmousemove=function(e){ //注册移动事件
				...
				conObg.scrollTop=(tops / magic(barid).offsetHeight * conObg.scrollHeight);
			};
	...
	ScrollPic:function(id,leftbut,rightbut,path,page,autoPlayTime,nowClass,rate){
		...
		var scrollId=magic(obj[0]);
		if(nowClass){page=1;magic(obj[0],"div")[0].innerHTML+=magic(obj[0],"div")[0].innerHTML;} //如果有当前样式，复制内容
		var scroll_List=magic(obj[0],obj[1]);
		...
		magic(leftbut).onmousedown=function(){xy="-";ScrollSpace();};
		magic(rightbut).onmousedown=function(){xy="+";ScrollSpace();};
