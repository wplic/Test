	...
	new Ajax.Request(url, {method:'post',postBody: paras,asynchronous: false,				 
				  onSuccess: function(response) {		             
								if(response.responseText.trim()=='false'){
									...
									b.action = "${contextPath}/registerAction.do?method=saveRegister2"
									b.submit(); 		
								}
						}, 
				onFailure: function() {alert("昵称注册验证失败！");}    
...
CustRegister custRegister=custRegisterService.getUserByServiceNameAndNotInId(tsystemWebuser.getUserName());
if (custRegister != null)
		{
				%><script type="text/javascript">
						...
						alert("您已经参加过报名了！");
						  ...
						  var url = "${contextPath}/registerAction.do?method=custInfo";
					...
					<div class="columnOut">								
						 ...
						 <div class="columnMain summary system">
						  <h3> 竞赛报名</h3>
							...
							<div class="tips" style="margin-left:117px">请输入您的竞赛专用昵称，以便您查看竞赛排名结果</div>							
								<div class="nickname">
									<label>客户昵称：</label>
									<input type="text" name="cnickName" id="cnickName" class="intotext""/>	
									<input type="button" name="Submit3" value="检测"  class="but_02" onclick="checkCpetName()"/>	
								</div>								
								<div class="referBox">
									<input type="button" onclick="checkCustonReg()" class="refebtnad1" />
									<input type="reset" value="" class="refebtnad2" />
								</div>
							</form>
						</div>							
