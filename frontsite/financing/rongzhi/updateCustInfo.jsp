...
function checkCpetName(){
	...
	new Ajax.Request(url, {method:'post',postBody: paras,asynchronous: false,				 
				  onSuccess: function(response) {		             
								if(response.responseText.trim()=='false'){
									...
									b.action = "${contextPath}/registerAction.do?method=updateRegister"
									b.submit(); 		
								}
						}, 
				onFailure: function() {alert("昵称注册验证失败！");}    
	...
	function gobuck(){
	window.location.href = "registerAction.do?method=custInfo";
}
</SCRIPT>
</head>
<%
			...
			<tr>
				...
				<td valign="top" class="wrapright">
					<div class="columnOut">				
								...
								<h3>信息修改</h3>
							<span><b>首页</b>><b>2012年广发证券“融智杯”融资融券仿真交易竞赛</b>><strong>信息修改</strong>
							</span>						
						     </div>
						 <div class="columnMain summary">
							...
							<div class="tips">请输入您的竞赛专用昵称，以便您查看竞赛排名结果</div>
							 <c:choose><c:when test="${custRegister!=null }">
								<div class="nickname">
									<label>客户昵称：</label>
									<input type="text" value="${custRegister.petName }" name="cnickName" id="cnickName" class="intotext" readonly />	
							  ...
							  <input type="button" value="确定" onclick="checkCustonReg()"class="but_03" />
