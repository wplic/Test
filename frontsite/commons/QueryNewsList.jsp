...
<%	
	...
	if(StartDate!=null&&StartDate.length()>0&&StartDate.length()==9){
		desc+="起始时间:"+StartDate+" | ";
	}
	if(EndDate!=null&&EndDate.length()>0&&StartDate.length()==9){
		desc+="结束时间:"+EndDate;
	}
	...
	//针对参数值 .y 来过滤
	String vakidateSqlString =request.getQueryString();	
    if(null!=vakidateSqlString && sqlInjectValidate.vakidateSqlStringForName(vakidateSqlString,".y"))
    {
		response.sendRedirect(request.getContextPath() +"/commons/error2.jsp");	
	}
