...
<div>
  <table width="100%" border="0">
    <tr>
      <td class="font14 bold lh30 font_st red">·持仓明细</td>
      <td align="right">
    <html:form action="/fund.do?method=getKeyStockPortfolioByCodeAndDate" method="post" styleId="fundForm">
...
<table border="1" class="lh24 center" bordercolor="#cccccc" width="100%" id="tabs11">
  <tr>
    ...
    <td width="15%" class="tab_th1 bold">持仓数量</td>
    <td width="17%" class="tab_th1 bold">持仓市值</td>
    <td width="37%" class="tab_th1 bold">持仓占净值比例</td>
    <td width="15%" class="tab_th1 bold">日期</td>
    </tr>
    <c:forEach items="${keyStockPortfolioList }" var="bean" varStatus="status">
              <tr class="wisdom_tab_lb">
                <td>
                	<c:choose>
						<c:when
							test="${empty bean.secName}">&nbsp;</c:when>
						<c:otherwise>
                ...
                <td>
                	<c:choose>
						<c:when
							test="${empty bean.sharesHolding}">&nbsp;</c:when>
						<c:otherwise>
							<bean:write name="bean" property="sharesHolding"  format="###,###"/>
						</c:otherwise>
					</c:choose>                
                </td>  
                   <td>
                	<c:choose>
						<c:when
							test="${empty bean.marketValue}">&nbsp;</c:when>
						<c:otherwise>
                   ...
                   <td>
                	<c:choose>
						<c:when
							test="${empty bean.ratioInNV}">&nbsp;</c:when>
						<c:otherwise>
...
<form action="/fund.do?method=getKeyStockPortfolioByCodeAndDate&gpdm=${fundCode}" method="post" id="relationForm">					
