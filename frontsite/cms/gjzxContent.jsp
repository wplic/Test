...
<%
	...
	if(doc!=null){
		...
		if(null!=_docUserType && null!=usertypelist){
			for(int i=0;i<usertypelist.size();i++){
				Dictionary dic = (Dictionary) usertypelist.get(i);
				if(_docUserType.longValue()==dic.getId().longValue()){
					docUserType = Long.parseLong(dic.getKeyShortDesc());
					docUserTypeStr = dic.getKeyName();
				}
			}
		}
	...
	if(tsystemwebuserlevel.longValue()<docUserType.longValue()){
%>
	<script type="text/javascript">
		alert ("对不起，您为<%=tsystemwebuserlevelstr%>，没有权限浏览<%=docUserTypeStr%>资讯文章，请尽快升级！") ;
...
<div class="main">
	...
	<p class="textTitle2" align="center">信息来源：<%= DocumentHelper.getAttrValue(doc ,"source" , "") %> 日期：<%= DocumentHelper.getUploadDate(doc ,"yyyy-MM-dd")%> 作者：<%= DocumentHelper.getAttrValue(doc ,"author" , "") %> </p>
    <%if(!baseBrief.equals("")) {%>
	...
	//if(whichCat.indexOf("zqyj_yjbg")==0){
		...
		<%}else{%>
			<p class="textTitle1" align="center">
				对不起，您为<%=tsystemwebuserlevelstr%>，没有权限浏览<%=docUserTypeStr%>资讯文章正文和附件，请尽快升级！</p>
		<%}%>
</div>
